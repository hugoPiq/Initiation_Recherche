...
with open(logger.get_dir() + "/parameters.txt", 'w') as out:
    out.write(
        ...
        + 'timesteps_per_batch = ' +
        str(alg_kwargs['timesteps_per_batch']) + '\n'
        ...
        str(alg_kwargs['normalize_observations']) + '\n'
...
network = mlp(num_layers=alg_kwargs['num_layers'],
              num_hidden=alg_kwargs['num_hidden'], layer_norm=alg_kwargs['layer_norm'])

if transfer_path is not None:
    # Do transfer learning
    _ = trpo_mpi.learn(env=env, network=network,
                       load_path=transfer_path, **alg_kwargs)
else:
