...
defaults = get_learn_function_defaults('ppo2', 'my_robot_lstm')
...
try:
    logdir = defaults['trained_path'].split(
        'checkpoints')[0] + 'results' + defaults['trained_path'].split('checkpoints')[1]
except:
...
csv_files = [csvdir + "det_obs.csv", csvdir +
             "det_acs.csv", csvdir + "det_rew.csv"]
...
def make_model(): return ppo2.Model(policy=policy, ob_space=ob_space, ac_space=ac_space, nbatch_act=nenvs,
...
while loop:
    ...
    # lock if less than 5mm error in each axis
    if np.allclose(obs[0][6:9], np.asarray([0., 0., 0.]), atol=0.005):
