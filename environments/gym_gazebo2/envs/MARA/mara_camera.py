...
class MARACameraEnv(gym.Env):
    ...
    def __init__(self):
        ...
        self.targetPosition = np.asarray(
            [-0.40028, 0.095615, 0.72466])  # close to the table
        ...
        self._sub_coll = self.node.create_subscription(
            ContactState, '/gazebo_contacts', self.collision_callback, qos_profile=qos_profile_sensor_data)
        ...
        self.mara_chain = self.ur_tree.getChain(
            self.environment['linkNames'][0], self.environment['linkNames'][-1])
    ...
    def collision_callback(self, message):
        ...
        """
        collision_messages = [
            "mara::base_robot::base_robot_collision", "ground_plane::link::collision"]
